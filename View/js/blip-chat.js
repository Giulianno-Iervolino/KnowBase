...
var CREATE_ACCOUNT_EVENT = 'OnCreateAccount';
...
var BlipChat = exports.BlipChat = function () {
  ...
  _createClass(BlipChat, [{
    ...
    key: 'withAccount',
    value: function withAccount(account) {
    ...
    value: function build() {
      this.widget = new _BlipChatWidget.BlipChatWidget(this.appKey, this.buttonConfig, this.authConfig, this.account, this.target, this.events, this.environment || "production");
    }
  }, {
    key: 'toogleChat',
    value: function toogleChat() {
...
var _chat = __webpack_require__(132);

var _chat2 = _interopRequireDefault(_chat);

var _brandLogo = __webpack_require__(133);
...
var _StorageService = __webpack_require__(135);

var _StorageService2 = _interopRequireDefault(_StorageService);

var _NotificationHandler = __webpack_require__(136);

var _Misc = __webpack_require__(138);
...
var BlipChatWidget = exports.BlipChatWidget = function () {
  function BlipChatWidget(appKey, buttonConfig, authConfig, account, target, events, environment) {
    _classCallCheck(this, BlipChatWidget);

    self = this;
    self.appKey = appKey;
    self.buttonColor = buttonConfig && buttonConfig.color ? buttonConfig.color : '#2CC3D5';
    self.buttonIcon = buttonConfig && buttonConfig.icon ? buttonConfig.icon : _brandLogo2.default;
    self.authConfig = self._parseAuthConfig(authConfig);
    self.account = self._addAuthTypeToExtras(account, authConfig);
    self.target = target;
    self.events = events;
    self.blipChatContainer = target || _Misc.dom.createDiv('#blip-chat-container');
    self.isOpen = false;
    ...
    self.NotificationHandler = new _NotificationHandler.NotificationHandler(self);
  ...
  _createClass(BlipChatWidget, [{
    ...
    value: function _onInit() {
      var rendered = _Misc.dom.render(_chat2.default, this);
    ...
    key: '_setSubscribers',
    value: function _setSubscribers() {
      // Subscribe update count
      var updateNotifications = function updateNotifications(count) {
        return document.getElementById('blip-chat-notifications').textContent = count;
      };
      self.NotificationHandler.subscribe(updateNotifications);

      // Subscribe update style
      var toggleNotificationsButton = function toggleNotificationsButton(count) {
        return document.getElementById('blip-chat-notifications').style.opacity = count > 0 ? 1 : 0;
      };
      self.NotificationHandler.subscribe(toggleNotificationsButton);
    }
  }, {
    key: '_setChatUrlEnvironment',
    value: function _setChatUrlEnvironment(environment, authConfig, appKey) {
    ...
    value: function _addAuthTypeToExtras(account, authConfig) {
      var authType = authConfig ? authConfig.authType || _BlipChat.BlipChat.GUEST_AUTH : _BlipChat.BlipChat.GUEST_AUTH;
      if (account) {
        account.extras = account.extras || {};
        account.extras.authType = authType;
      } else {
        account = {
          extras: { authType: authType }
        };
      }
    ...
    value: function _createIframe() {
      ...
      self.blipChatIframe.onload = function () {
        var userAccount = self._getObfuscatedUserAccount();
        self._sendPostMessage({ code: _Constants2.default.START_CONNECTION_CODE, userAccount: userAccount });
      };
    ...
    value: function _sendPostMessage(data) {
      var blipChatIframe = document.getElementById('blip-chat-iframe');
    ...
    key: '_openChat',
    value: function _openChat(event, forceClose) {
      var blipChatIcon = document.getElementById('blip-chat-icon');
      var blipChatCloseIcon = document.getElementById('blip-chat-close-icon');
      var blipChatButton = document.getElementById('blip-chat-open-iframe');
      ...
      if (!forceClose && self.blipChatIframe && !self.blipChatIframe.classList.contains('blip-chat-iframe-opened')) {
        ...
        var meta = document.getElementById('blipChatMeta');
    ...
    value: function _onReceivePostMessage(message) {
      switch (message.data.code) {
        ...
        case _Constants2.default.CREATE_ACCOUNT_CODE:
          ...
          if (accountObj.authType === _Constants2.default.GUEST_AUTH) {
            _StorageService2.default.setToLocalStorage(_Constants2.default.USER_ACCOUNT_KEY, accountObj, _Constants2.default.COOKIES_EXPIRATION);
          }
          break;

        case _Constants2.default.CHAT_CONNECTED_CODE:
          if (self.account) self._sendPostMessage({ code: _Constants2.default.USER_IRIS_ACCOUNT, account: self.account });

          if (self.events.OnLoad) self.events.OnLoad();

          if (self.pendings) {
            self.pendings.map(function (pending) {
              if (pending.content) {
                // If is a message
    ...
    key: 'sendMessage',
    value: function sendMessage(userMessage) {
      // Process Message before sending
      var content = void 0;
      if ((typeof userMessage === 'undefined' ? 'undefined' : _typeof(userMessage)) === 'object') {
        if (!userMessage.payload) {
          ...
          if (userMessage.preview) {
            content.metadata = {
              '#blip.payload.content': typeof userMessage.preview.content === 'string' ? userMessage.preview.content : JSON.stringify(userMessage.preview.content),
      ...
      if (!self.isChatLoaded) {
        self.pendings.push({ content: content });
        self._createIframe();
        return;
      }
      self._sendPostMessage({ code: _Constants2.default.SEND_MESSAGE_CODE, content: content });
    }
  }, {
    key: 'sendCommand',
    value: function sendCommand(command) {
      // If chat is not connected, connect it and wait to send command
      if (!self.isChatLoaded) {
        self.pendings.push({ command: command });
        self._createIframe();
        return;
      }
      self._sendPostMessage({ code: _Constants2.default.SEND_COMMAND_CODE, command: command });
    }
  }, {
    key: 'destroy',
    value: function destroy() {
...
var SEND_COMMAND_CODE = 'SendCommand';
var USER_IRIS_ACCOUNT = 'UserIrisAccount';
var USER_ACCOUNT_KEY = 'blipSdkUAccount';
var PARENT_NOTIFICATION_CODE = 'NewBotMessage';
var SHOW_CLOSE_BUTTON = 'ShowCloseButton';
var CLOSE_WIDGET = 'CloseWidget';
...
var CONSTANTS = {
  ...
  SEND_COMMAND_CODE: SEND_COMMAND_CODE,
  USER_IRIS_ACCOUNT: USER_IRIS_ACCOUNT,
  ...
  PARENT_NOTIFICATION_CODE: PARENT_NOTIFICATION_CODE,
...
var _Observable = __webpack_require__(137);

function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }

var NotificationHandler = exports.NotificationHandler = function () {
  function NotificationHandler(context) {
    var _this = this;

    _classCallCheck(this, NotificationHandler);

    this.context = context;
    this.observer = new _Observable.Observable();
    this.notifications = [];
    this.originalDocumentTitle = document.title;

    //
    window.document.addEventListener('visibilitychange', function () {
      return _this.resetDocumentTitle();
    });
  }
  ...
  _createClass(NotificationHandler, [{
    ...
    value: function sendDocumentTitleNotification(botName) {
      var _this2 = this;

      var count = 0;
      if (!this.documentTitleInterval) {
        this.documentTitleInterval = setInterval(function () {
          document.title = count % 2 ? botName + ' diz...' : _this2.originalDocumentTitle;
          count++;
        }, 300);
      }
    }
    ...
    value: function resetDocumentTitle() {
      if (!window.document.hidden) {
        // if tab is being hidden
    ...
    value: function passOnConditions() {
...
var dom = exports.dom = {
  createDiv: function createDiv(selector) {
